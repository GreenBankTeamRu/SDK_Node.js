...
class ApiClient {
    ...
    _loadTrustStore() {
        const certs = [];
        const caPaths = this.config.caPath;
        if (!caPaths) return certs;

        const pathsArray = Array.isArray(caPaths) ? caPaths : [caPaths];

        for (const caPath of pathsArray) {
            const buf = fs.readFileSync(caPath);
            const text = buf.toString('utf8');

            if (text.includes('-----BEGIN CERTIFICATE-----')) {
                const caCerts = this._parsePemCertificates(text);
                certs.push(...caCerts.map(cert => forge.pki.certificateToPem(cert)));
            } else {
                // DER (.cer/.crt в DER)
                const derBinary = buf.toString('binary'); // важно: binary, не utf8 [web:22]
