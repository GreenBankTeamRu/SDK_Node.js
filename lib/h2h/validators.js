...
const linkDataRequestSchema = Joi.object({
  linkType: Joi.string()
    .valid(LinkTypeEnum.ONE_TIME, LinkTypeEnum.REUSABLE)
    .messages({
      'any.only': 'Тип ссылки должен быть одним из: oneTime, reusable'
    }),
  account: Joi.string()
    .pattern(/^[0-9]{20}$/)
    .max(20)
    .required()
    .messages({
      'string.pattern.base': 'Номер счета должен содержать ровно 20 цифр',
      'string.max': 'Номер счета не должен превышать 20 символов',
      'any.required': 'Номер счета обязателен'
    }),
  amount: Joi.string()
    .pattern(/^[0-9]{1,12}$/)
    .max(12)
    .messages({
      'string.pattern.base': 'Сумма должна содержать от 1 до 12 цифр',
      'string.max': 'Сумма не должна превышать 12 символов'
    }),
  takeTax: Joi.boolean(),
  totalTaxAmount: Joi.string()
    .pattern(/^[0-9]{1,12}$/)
    .max(12)
    .messages({
      'string.pattern.base': 'Сумма НДС должна содержать от 1 до 12 цифр',
      'string.max': 'Сумма НДС не должна превышать 12 символов'
    }),
  paymentPurpose: Joi.string()
    .max(210)
    .messages({
      'string.max': 'Назначение платежа не должно превышать 210 символов'
    }),
  dayLife: Joi.string()
  ...
  linkName: Joi.string()
    ...
    .max(100)
    .messages({
      'string.pattern.base': 'Название содержит недопустимые символы',
  ...
  redirectUrl: Joi.string()
    ...
    .max(1024)
    .messages({
      'string.pattern.base': 'URL должен начинаться с http:// или https://',
      'string.max': 'URL не должен превышать 1024 символа'
    }),
  brandName: Joi.string()
    .max(35)
    .messages({
      'string.max': 'Наименование ТСП не должно превышать 35 символов'
    }),
  payerInn: Joi.string()
    .pattern(/^[0-9]{10,12}$/)
    .max(50)
    .messages({
      ...
      'string.max': 'ИНН не должен превышать 50 символов'
    })
});

const sbpB2BLinkCreateRequestSchema = Joi.object({
  linkData: linkDataRequestSchema
    .required()
    .messages({
      'any.required': 'Данные ссылки обязательны',
      'object.base': 'Данные ссылки должны быть объектом'
    }),
  email: Joi.array()
    .items(
      Joi.string()
    ...
    .max(3)
    .messages({
      'array.max': 'Список email не должен превышать 3 элемента'
    })
});

export default { certRequestReqSchema, certRequestReqEIOSchema, paymentReqSchema, payrollReqSchema, sbpB2BLinkCreateRequestSchema };
